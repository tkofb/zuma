...
double FuturesTrade::getRealizedProfitAndLoss() {
...
FuturesInfo::Subtotal::Subtotal(string symbol, double quantity, double realized, double markToMarket)
    : symbol(symbol),
      quantity(quantity),
      realizedProfitAndLoss(realized),
      markToMarketProfitAndLoss(markToMarket) {}

double FuturesInfo::Subtotal::getQuantity() const {
    return quantity;
}
...
FuturesInfo::FuturesInfo(vector<FuturesTrade> trades, vector<Subtotal> subtotals, double realized, double markToMarket)
    : trades(trades),
      subtotals(subtotals),
      realizedProfitAndLoss(realized),
      markToMarketProfitAndLoss(markToMarket),
