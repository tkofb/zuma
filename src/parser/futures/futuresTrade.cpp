...
string FuturesInfo::Subtotal::getSymbol() const { return symbol; }
...
FuturesInfo::FuturesInfo()
    : realizedProfitAndLoss(0), markToMarketProfitAndLoss(0),
      profitableTrades(0), unprofitableTrades(0), gainsFromTrades(0),
      lossesFromTrades(0), avgWin(0), avgLoss(0), totalTrades(0), winRate(0.0) {
}

FuturesInfo::FuturesInfo(vector<FuturesTrade> trades,
                         vector<Subtotal> subtotals, double realized,
                         double markToMarket)
    : trades(trades), subtotals(subtotals), realizedProfitAndLoss(realized),
      markToMarketProfitAndLoss(markToMarket), profitableTrades(0),
      unprofitableTrades(0), gainsFromTrades(0), lossesFromTrades(0), avgWin(0),
      avgLoss(0), totalTrades(0), winRate(0.0) {
...
void FuturesInfo::updateNumberOfProfitableTrades() {
  ...
  winRate = totalTrades > 0
                ? static_cast<double>(profitableTrades) / totalTrades
                : 0.0;
}
...
void FuturesInfo::print() {
  ...
  cout << "Total"
       ...
       << " | Win Rate: " << winRate << " | AVG Loss: " << avgLoss
       << " | AVG Win: " << avgWin
    ...
    if (statementType == "Trades") {
      ...
      if (assetType == "Futures") {
        ...
        if (parsedTrade[2] == "Order") {
          ...
          FuturesInfo info(listOfFuturesTrades, listOfFuturesSubtotals,
                           realized, mark);
          result = info;
        }
      }
